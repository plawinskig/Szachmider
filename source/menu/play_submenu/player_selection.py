...
class PlayerSelection():
    ...
    def check_for_input(self, position):
        ...
        for btn in self.BTN_PLAYER_LIST:
            if btn.check_for_input(position):
                if not self.containsEquals and i-3 == len(self.BTN_PLAYER_LIST):
    ...
    def filtrate_player_list(self, text):
        ...
        for player in self.player_list:
            ...
            if text.strip().lower() == player.lower():
                is_equal = True
        self.containsEquals = is_equal
        self.set_player_list(filtred_players)
